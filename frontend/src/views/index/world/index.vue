...
<template>
  <div class="flex flex-col gap-4 p-2">
    <div class="flex justify-center items-center flex-wrap gap-4">
      <magic-card class="w-full sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1rem)] xl:w-[calc(18%-1rem)] h-64 sm:h-72 md:h-96 lg:h-118" v-for="item in page.data" :key="item.id" :description="item.description"
        :labels="item.labels" :title="item.nickname" @click="$router.push(`/world/${item.id}`)" />
