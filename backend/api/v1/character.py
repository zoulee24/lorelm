...
from backend.crud import CharacterCrud, LabelCrud, UserCrud
...
@role_router.post("/")
async def role_info(
    form: Annotated[schemas.CharacterCreateForm, Form()], db: dependencies.DependSession
):
    crud = CharacterCrud(db)
    exist = await crud.get_count(wheres=crud.model.nickname == form.nickname)
    if exist != 0:
...
@role_router.get("/label", response_model=PageResponse[schemas.LabelResponse])
...
@role_router.get("/label/stream", response_model=list[schemas.LabelResponse])
async def label_list(
...
@role_router.post(
    "/label", summary="创建标签", response_model=list[schemas.LabelResponse]
)
async def label_create(
    data: Annotated[list[str], Body(description="标签")], db: dependencies.DependSession
):
    crud = LabelCrud(db)
...
@role_router.delete("/label/{label_id}", summary="删除标签")
